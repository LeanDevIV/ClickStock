...
export const useTableData = (section, options = {}) => {
  ...
  const handleRestore = useCallback(async (id) => {
    ...
    try {
      ...
      setData((prev) =>
        prev.map((item) =>
          item._id === id
            ? productoRestaurado.producto || { ...item, isDeleted: false, ...(section === 'Productos' ? { disponible: true, deletedBy: null, deletedAt: null } : {}) }
            : item
        )
      );
      
      console.log('Elemento restaurado:', id, productoRestaurado.producto?.nombre || productoRestaurado.nombre);
    } catch (err) {
  ...
  const handleSoftDelete = useCallback(async (id) => {
    try {
      const endpoint = config.softDeleteEndpoint?.replace(':id', id) || config.updateEndpoint.replace(':id', id);
      // Usar DELETE para soft delete (el backend captura el usuario desde el token)
      const { data: productoActualizado } = await clientAxios.delete(endpoint);

      // Usar la respuesta del servidor que incluye deletedBy y deletedAt
      setData((prev) =>
        prev.map((item) =>
          item._id === id ? productoActualizado.producto || { ...item, isDeleted: true, ...(section === 'Productos' ? { disponible: false } : {}) } : item
        )
      );

      console.log('Producto marcado como eliminado (soft):', id, productoActualizado);
    } catch (err) {
  ...
  const handleHardDelete = useCallback(async (id) => {
    try {
      ...
      if (!endpoint) {
        throw new Error('No hay endpoint configurado para eliminar permanentemente');
      }
